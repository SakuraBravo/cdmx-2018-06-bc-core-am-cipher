//function introducirTexto(){ //ES5
//    let x = document.getElementById("entrada").value;
//    let y = document.getElementById("offset").value;
  ...
  if (true){
   let i = 0;// Esta es la funcion que mide el string y lo separa en índices
    while (i < intro.length) {
      var altas = (((intro[i]).toUpperCase())).charCodeAt();//Variable con codigo ASCII
   ...
   const codigo = codificar (altas, offset); //Declaro los argumentos que se aplican a la función que codifica el mensaje
